...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        for i in self.bids:
            if(sequence_number - self.importantorders[i] > 200):
                self.importantorders.pop(i)
        for i in self.asks:
            if(sequence_number - self.importantorders[i] > 200):
                self.asks.remove(i)
                self.importantorders.pop(i)
            ...
            if(ask_prices[0] - bid_prices[0] > 2*TICK_SIZE_IN_CENTS):
                ...
                if(goingup):
                    ...
                    if self.bid_id == 0 and new_bid_price != 0 and self.position < POSITION_LIMIT - 10:
                        ...
                        self.importantorders[self.bid_id]=sequence_number
                    ...
                    if self.ask_id == 0 and new_ask_price != 0 and self.position > -POSITION_LIMIT + 10:
                        ...
                        self.importantorders[self.ask_id]=sequence_number
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            if(self.importantorders.get(client_order_id) != None):
                for i in range(0, 5):
                    self.send_insert_order(next(self.order_ids), Side.ASK, 0, global_ask_Vol[i], Lifespan.FILL_AND_KILL)
            ...
            if(self.importantorders.get(client_order_id) != None):
                for i in range(0, 5):
                    self.send_insert_order(next(self.order_ids), Side.BID, 2**31-1 ,global_bid_Vol[i], Lifespan.FILL_AND_KILL)
